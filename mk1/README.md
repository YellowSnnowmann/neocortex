# Neocortex Mark 1 (MK1, Feb 2026)

First Neocortex Mark 1 release (February 2026). Invoke via the gateway using model id **`mk1`**. See the [repository README](../README.md) for base URL and endpoints.
...
The model runs behind the Neocortex/tinyhuman gateway at `https://api.tinyhuman.xyz`. Use **OpenAI-style model routing** by setting `"model": "mk1"` in the request body (e.g. `POST /webhook`). Default provider and backing LLM are set in gateway config; “Mark 1” (MK1) refers to the gateway + prompt + tool stack for this release.
