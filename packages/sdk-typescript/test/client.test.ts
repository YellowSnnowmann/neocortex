...
describe('TinyHumanMemoryClient', () => {
  ...
  describe('constructor', () => {
    it('throws when token is missing', () => {
      expect(() => new TinyHumanMemoryClient({ token: '' })).toThrow('token is required');
      expect(() => new TinyHumanMemoryClient({ token: '   ' })).toThrow('token is required');
    });
  ...
  describe('error handling', () => {
    it('throws TinyHumanError on non-ok response with error message', async () => {
      fetchMock.mockResolvedValueOnce({
        ok: false,
        status: 400,
      ...
      try {
        ...
        expect(e).toBeInstanceOf(TinyHumanError);
        expect((e as TinyHumanError).message).toBe('Bad request');
        expect((e as TinyHumanError).status).toBe(400);
        expect((e as TinyHumanError).body).toEqual({ success: false, error: 'Bad request' });
      }
    });
